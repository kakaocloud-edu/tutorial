...
아래 명령어를 실행하면 1초마다 CDN VM에 같은 파일을 요청하면서 `X-Cache-Status` 값을 확인할 수 있습니다.

```bash
while true; do printf "[%s] " "$(date '+%F %T')"; curl -sI http://CDN_VM_PUBLIC_IP/index2.html | grep -i '^X-Cache-Status:'; sleep 1; done
```

`CDN_VM_PUBLIC_IP`는 본인의 CDN VM Public IP로 변경합니다.

예상 출력 예시는 다음과 같습니다.

```text
[2026-07-08 10:15:01] X-Cache-Status: MISS
[2026-07-08 10:15:02] X-Cache-Status: HIT
[2026-07-08 10:15:03] X-Cache-Status: HIT
...
```

처음 요청은 캐시에 파일이 없기 때문에 `MISS`가 표시될 수 있습니다. 이후 같은 파일을 다시 요청하면 캐시에 저장된 응답을 사용하므로 `HIT`가 표시됩니다.

TTL이 60초로 설정되어 있다면, 캐시된 응답은 60초 동안 유효합니다. TTL이 지난 뒤 다시 요청하면 Nginx는 Origin에 다시 요청하거나 캐시를 갱신할 수 있습니다. 이때 상황에 따라 `MISS`, `EXPIRED`, `REVALIDATED`, `STALE` 등의 값이 나타날 수 있습니다.

명령어 실행을 중단하려면 `Ctrl + C`를 누릅니다.
