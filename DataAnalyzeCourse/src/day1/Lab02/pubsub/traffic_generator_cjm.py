...
def pick_preferred_product_id(gender: str, age_segment: str) -> str:
    ...
    cat_list = config_cjm.CATEGORY_PREFERENCE.get(gender, {}).get(age_segment, [])
...
def try_register(session: requests.Session, user_id: str, gender: str, age_segment: str) -> bool:
    ...
    try:
        url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["ADD_USER"]
...
def try_login(session: requests.Session, user_id: str) -> bool:
    ...
    try:
        url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["LOGIN"]
        r = session.post(url, data=payload, headers=headers)
...
def try_logout(session: requests.Session, user_id: str) -> bool:
    headers = {"Accept": "application/json"}
    try:
        url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["LOGOUT"]
...
def try_delete_user(session: requests.Session, user_id: str) -> bool:
    ...
    payload = {"user_id": user_id}
    try:
        url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["DELETE_USER"]
        r = session.post(url, data=payload, headers=headers)
...
def do_anon_sub_fsm(session: requests.Session, user_unique_id: str):
    ...
    while sub_state != "Anon_Sub_Done":
        ...
        if sub_state not in config_cjm.ANON_SUB_TRANSITIONS:
            logging.warning(f"[{user_unique_id}] {sub_state} not in ANON_SUB_TRANSITIONS => break")
            break

        transitions = config_cjm.ANON_SUB_TRANSITIONS[sub_state]
        if not transitions:
        ...
        time.sleep(random.uniform(*config_cjm.TIME_SLEEP_RANGE))
...
def perform_anon_sub_action(session: requests.Session, user_unique_id: str, sub_state: str):
    ...
    if sub_state == "Anon_Sub_Main":
        try:
            r = session.get(config_cjm.API_URL_WITH_HTTP, headers=headers)
        ...
        url = f"{config_cjm.API_URL_WITH_HTTP}{config_cjm.API_ENDPOINTS['PRODUCT_DETAIL']}?id={pid}"
        ...
        if categories_cache:
            ...
            cat_url = f"http://{config_cjm.API_BASE_URL}/{config_cjm.API_ENDPOINTS['CATEGORY']}?name={chosen_cat}"
            try:
        ...
        q = random.choice(config_cjm.SEARCH_KEYWORDS)
        search_url = f"http://{config_cjm.API_BASE_URL}/{config_cjm.API_ENDPOINTS['SEARCH']}?query={q}"
        try:
        ...
        try:
            err_url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["ERROR_PAGE"]
...
def do_logged_sub_fsm(session: requests.Session,
    ...
    while sub_state != "Login_Sub_Done":
        ...
        if sub_state not in config_cjm.LOGGED_SUB_TRANSITIONS:
            logging.warning(f"[{user_unique_id}] {sub_state} not in LOGGED_SUB_TRANSITIONS => break")
            break

        transitions = config_cjm.LOGGED_SUB_TRANSITIONS[sub_state]
...
def perform_logged_sub_action(session: requests.Session,
    ...
    if sub_state == "Login_Sub_Initial":
        ...
        if categories_cache:
            # 선호 카테고리 기반으로 선택
            preferred_cats = config_cjm.CATEGORY_PREFERENCE.get(gender, {}).get(age_segment, [])
        ...
        url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["CART_VIEW"]
        try:
            r = session.get(url, headers=headers)
        ...
        url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["CHECKOUT_HISTORY"]
        try:
            r = session.get(url, headers=headers)
        ...
        try:
            add_url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["CART_ADD"]
        ...
        view_url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["CART_VIEW"]
        try:
            ...
            if vr.status_code == 200:
                ...
                if items:
                    ...
                    remove_url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["CART_REMOVE"]
        ...
        check_url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["CHECKOUT"]
        try:
        ...
        try:
            rev_url = config_cjm.API_URL_WITH_HTTP + config_cjm.API_ENDPOINTS["ADD_REVIEW"]
...
def run_user_simulation(user_idx: int):
    ...
    device_type = random.choice(config_cjm.DEVICE_TYPES)
    ...
    if existing_user_ids and random.random() < config_cjm.REVISIT_PROBABILITY:
        user_unique_id = random.choice(list(existing_user_ids))
    else:
        user_unique_id = f"user_{uuid.uuid4().hex[:6]}"
        existing_user_ids.add(user_unique_id)

    pareto_sample = random.paretovariate(config_cjm.SESSION_LENGTH_SHAPE)
    actions_per_user = max(1, int(config_cjm.ACTIONS_PER_USER * pareto_sample))
    ...
    actions_per_user = max(1, int(actions_per_user * weight))
    
    min_sleep, max_sleep = config_cjm.TIME_SLEEP_RANGE
    for e in config_cjm.SALE_EVENTS:
        if e["start_hour"] <= hour < e["end_hour"]:
            weight *= e["bonus_factor"]
    time_sleep_range = (min_sleep / weight, max_sleep / weight)
    ...
    while True:
        ...
        if current_state not in config_cjm.STATE_TRANSITIONS:
            logging.error(f"[{user_unique_id}] no transitions from {current_state} => end.")
            break

        possible_next = config_cjm.STATE_TRANSITIONS[current_state]
...
def launch_traffic(num_users, max_threads, time_sleep_range):
    ...
    orig_num_users       = config_cjm.NUM_USERS
    orig_max_threads     = config_cjm.MAX_THREADS
    orig_time_sleep_range = config_cjm.TIME_SLEEP_RANGE
    config_cjm.NUM_USERS     = num_users
    config_cjm.MAX_THREADS   = max_threads
    config_cjm.TIME_SLEEP_RANGE = time_sleep_range

    # 미리 상품·카테고리 캐시 갱신
    fetch_products(config_cjm.API_URL_WITH_HTTP)
    fetch_categories(config_cjm.API_URL_WITH_HTTP)
    ...
    # 원래 config 값 복원
    config_cjm.NUM_USERS       = orig_num_users
    config_cjm.MAX_THREADS     = orig_max_threads
    config_cjm.TIME_SLEEP_RANGE = orig_time_sleep_range


if __name__ == "__main__":
    args = parse_args()
    if args.mode == "once":
        launch_traffic(config_cjm.NUM_USERS, config_cjm.MAX_THREADS, config_cjm.TIME_SLEEP_RANGE)
        ...
        while True:
            ...
            for pattern in patterns:
                ...
                num_users   = max(1, int(config_cjm.NUM_USERS   * factor))
                max_threads = max(1, int(config_cjm.MAX_THREADS * factor))

                # 3) 액션 딜레이 범위에도 ±20% jitter 적용
                base_min, base_max = config_cjm.TIME_SLEEP_RANGE
