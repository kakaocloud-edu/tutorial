...
config_path = os.path.abspath(
    os.path.join(current_dir, "..", "..", "Lab01", "traffic_generator")
)
...
if __name__ == "__main__":
    ...
    if args.mode == "once":
        ...
        while True:
            now = datetime.now()
            segment = get_time_segment(now.hour)
            min_f, max_f = SEGMENT_FACTORS[segment]
            ...
            logging.info(
                f"[Continuous][{segment}] factor={factor:.2f}, "
                f"users={num_users}, threads={max_threads}, "
                f"sleep_range=({sleep_min:.2f},{sleep_max:.2f})"
            )

            launch_traffic(num_users, max_threads, time_range)
            logging.info(f"[Continuous][{segment}] done. sleeping 5s before next segment")
            time.sleep(5)
