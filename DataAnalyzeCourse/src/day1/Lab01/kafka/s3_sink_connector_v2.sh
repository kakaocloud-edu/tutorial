...
curl "https://awscli.amazonaws.com/awscliv2-exe-linux-x86_64-${AWS_CLI_VERSION}.zip" -o "${AWS_CLI_ZIP}" || { echo "kakaocloud: AWS CLI 다운로드 실패"; exit 1; }
...
EOF
if [ $? -ne 0 ]; then echo "kakaocloud: nginx-s3-sink-connector.json 생성 실패"; exit 1; fi
sudo chown ubuntu:ubuntu "/home/ubuntu/kafka/config/connectors/nginx-s3-sink-connector.json" || { echo "kakaocloud: nginx-s3-sink-connector.json 소유권 변경 실패"; exit 1; }
