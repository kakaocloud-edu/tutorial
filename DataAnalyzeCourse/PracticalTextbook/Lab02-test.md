...
2. 인스턴스 생성 버튼 클릭
    ...
    - 고급 설정
        - 아래 스크립트 입력
            ...
            # (확인용) 현재 .bashrc 끝에 잘 들어갔는지 출력
            log "==== [DEBUG] Tail of /home/ubuntu/.bashrc ===="
            tail -n 20 /home/ubuntu/.bashrc
            ...
            # 체크
            log "현재 스크립트 셸에서 S3_ACCESS_KEY=$S3_ACCESS_KEY"
            log "현재 스크립트 셸에서 CONFLUENT_HOME=$CONFLUENT_HOME"
            log "현재 스크립트 셸에서 PATH=$PATH"
            ...
            # 만약 여전히 'command not found'가 뜬다면, 아래처럼 절대경로 사용 또는
            # export PATH="~/.local/bin:$PATH" 처리가 필요할 수 있음
            confluent-hub install confluentinc/kafka-connect-s3:latest \
              --component-dir /confluent-hub/plugins \
              --worker-configs /home/ubuntu/kafka/config/connect-standalone.properties \
              --no-prompt || log "WARN: confluent-hub install failed (command not found?)"
            ...
            ################################################################################
            log "Step 9: AWS CLI configure 파일에 자동 세팅"
            ...
            log "다시 확인 - AWS CLI 버전: $AWS_VERSION"
            log "Step 9: AWS CLI configure 설정 완료"
            source /home/ubuntu/.bashrc
