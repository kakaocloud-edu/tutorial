# Hadoop 환경에서 PySpark 기반 실시간/배치 데이터 통합 정제 및 사용자 행동 예측 데이터셋 구축
...
2. `core-hadoopMST` 상태 Actice 확인 후 인스턴스의 우측 메뉴바 > `Public IP 연결` 클릭
    - `새로운 퍼블릭 IP를 생성하고 자동으로 할당`
    - 확인 버튼 클릭
3. `core-hadoopMST` 인스턴스의 우측 메뉴바 > `SSH 연결` 클릭
    - SSH 접속 명령어 복사
    - 터미널 열기
