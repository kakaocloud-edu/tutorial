...
2. 클러스터 생성 버튼 클릭
    ...
    - 사용자 스크립트 (선택): [스크립트 사이트](http://210.109.83.141/) 를 활용하여 사용자 스크립트 다운로드 후 업로드 가능
        - 자신의 환경 변수에 맞게 수정 후 삽입
        - **사용자 스크립트**
            > MySQL·Kafka·Schema Registry 관련 환경 변수를 추가하고 로그를 남기며, 하둡 클러스터 노드들이 공통으로 사용할 환경 구성을 자동화
    ...
    - HDFS 설정
        - HDFS 블록 크기: `128MB`
        - HDFS 복제 개수: `2`
    - 클러스터 구성 설정 (선택): [스크립트 사이트](http://210.109.83.141/) 를 활용하여 사용자 스크립트 생성 가능
        - 자신의 환경 변수에 맞게 수정 후 삽입
        - **클러스터 구성 설정 파일**

            > 하둡 클러스터가 카카오클라우드 오브젝트 스토리지(S3 호환)에 접근할 수 있도록 인증 키와 리전, 엔드포인트 정보를 설정한 구성
            ```sql
            {
                "configurations":
                [
