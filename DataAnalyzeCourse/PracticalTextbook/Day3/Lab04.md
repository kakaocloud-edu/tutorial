...
실시간 정제된 데이터를 기반으로 하여 세션 이탈 추론 모델, 다음 상태 예측 모델, 성별 예측 모델 학습 과정입니다. 성별 예측 모델 학습 과정에서는 Katib에서 찾은 최적 하이퍼파라미터로 모델을 재학습하여 검증합니다. 이후 검증된 모델을 배포하여 추론 실습까지 진행합니다.
