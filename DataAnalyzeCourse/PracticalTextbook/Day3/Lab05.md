...
실시간 정제된 데이터를 기반으로 하여 성별 예측 모델 학습 과정입니다. 정제된 데이터를 전처리하여 모델 학습에 최적화 합니다. 이후 학습 및 검증을 마친 성별 추론 모델을  Object Storage에 저장하고 KServe를 이용하여 모델을 배포합니다. 배포된 모델에 샘플 데이터를 입력하여 성별 추론을 진행하는 실습입니다.
