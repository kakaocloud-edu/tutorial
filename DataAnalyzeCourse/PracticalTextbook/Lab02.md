...
2. 클러스터 생성 버튼 클릭
    ...
    - 브로커 구성 설정
        ...
        - 최대 IOPS: `3000`
    - 생성 버튼 클릭
3. API Server VM의 `{Kafka 클러스터 부트스트랩 서버}` 값을 실제 Kafka 클러스터 부트스트랩 서버 값으로 수정

    #### lab2-6-1-1
    
    ```bash
    sudo sed -i 's/{Kafka 클러스터 부트스트랩 서버}/실제 Kafka 클러스터 부트스트랩 서버값/g' /home/ubuntu/.bashrc /etc/default/logstash
    ```
