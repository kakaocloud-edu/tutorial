...
3. API Server VM의 `{임시 Kafka 클러스터 부트스트랩 서버}` 값을 실제 Kafka 클러스터 부트스트랩 서버 값으로 수정 -> api server 부분 나중에 따로 기본 환경 설정에 넣어서 수정 예정 / logs-to-kafka.conf로 path 수정하는 부분도 같이 넣기 
    ...
    ```bash
    sudo sed -i 's/{임시 Kafka 클러스터 부트스트랩 서버}/실제 Kafka 클러스터 부트스트랩 서버값/g' /home/ubuntu/.bashrc /etc/default/logstash
    ```
...
2. `traffic-generator-1`에서 Python 프로듀서 코드 실행
    ...
    ```bash
    sudo ln -s /DataAnalyzeCourse/src/day1/Lab02/kafka/kafka_publisher.py /opt/kafka/kafka_publisher.py
    ```
    ```bash
...
3. `traffic-generator-2`에서 Python 컨슈머 코드 실행
    ...
    ```bash
    sudo ln -s /DataAnalyzeCourse/src/day1/Lab02/kafka/kafka_subscriber.py /opt/kafka/kafka_subscriber.py
    ```
    ```bash
