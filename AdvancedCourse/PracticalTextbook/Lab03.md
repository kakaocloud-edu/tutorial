...
6. 사용자 스크립트 작성
   ...
   - 스크립트를 메모장에 복사후 복사해놓은 값들을 입력하기
     ...
     - 자바 버전(사용자 지정 입력)
      ...
      ### **Note**: 서버와 데이터베이스 통신시, 순서대로 진행하므로 꼭 database az-a의 엔드포인트, database az-b의 엔드포인트 순서대로 붙여넣어주세요. 
   ```bash
    #!/bin/bash
    ...
    export INPUT_DB_EP1='database az-a의 엔드포인트 입력'
    export INPUT_DB_EP2='database az-b의 엔드포인트 입력'
