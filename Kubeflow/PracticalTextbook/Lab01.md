...
#### pool-gpu 노드풀 생성 — CPU 전용 실습에서는 생략
   - **Note**: GPU 인스턴스(`p2i.6xlarge`)는 리전 재고/쿼터 이슈로 생성 실패가 잦았던 부분이라, CPU 전용 실습으로 전환하면서 이 노드풀 생성 자체를 스킵합니다.
7. 노드 풀 탭에서 생성된 노드 풀 목록 확인
    - pool-ingress, pool-worker 생성 확인
8. 노드 탭 클릭 후 노드 7개 생성 확인
