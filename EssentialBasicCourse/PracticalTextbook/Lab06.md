...
Load Balancer를 생성하고 2개의 Web 서버 VM과 연결합니다. Load Balancer가 트래픽을 잘 분배하는지 볼 수 있는 실습입니다. 추가로 DNS 설정을 진행합니다.
...
18. web_server_1, web_server_2의 우측 메뉴바 클릭 > Public IP 연결 해제 클릭

## 4. 웹 접속 및 밸런싱 여부 확인

1. 카카오 클라우드 콘솔 > 전체 서비스 > Load Balancing
2. App_LB_A의 Public IP 주소 복사
...
2. DNS 영역 생성 버튼 클릭
     - DNS 영역 이름 : `kakaocloud-edu.com`
       - **Note**: 실습 환경에 맞는 실제 보유 도메인으로 교체하세요.
3. 생성 버튼 클릭
4. 생성된 `kakaocloud-edu.com` DNS 영역 클릭
5. 레코드 생성 버튼 클릭
     - 유형 : `A`
     - TTL : `60`
     - 값 : `{App_LB_A의 Public IP}`
       - **Note**: "{App_LB_A의 Public IP}" 부분을 3번 섹션에서 App_LB_A에 연결한 실제 Public IP 주소로 교체하세요.
6. 생성 버튼 클릭
7. 네임서버 설정
     - 사용 도메인의 네임서버를 카카오클라우드의 네임서버(ns1.kakaocloud.com, ns2.kakaocloud.com)로 변경
     - 도메인 구입처의 도메인 설정창에서 네임서버를 변경
     - **Note**: 본 실습에서는 '가비아'라는 도메인 제공 서비스를 예시로 사용하였으며, 실제 사용 중인 도메인 제공업체의 네임서버 설정 화면에서 진행하면 됩니다.
8. 서비스 동작 확인
     - 브라우저에 연결한 도메인(예: `kakaocloud-edu.com`) 입력
     - Load Balancer는 web_server_1과 web_server_2에 연결되어 있기 때문에, 새로고침 시 호스트명이 연결된 VM의 정보로 바뀌는 것을 확인
     - 새로고침 시 바로 바뀌어서 접속되지 않고, 일정 시간 후 새로고침을 진행해야 함
